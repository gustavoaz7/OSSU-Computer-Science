...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift (dx, dy, exp) => let val exp_val = eval_prog(exp, env) in
	  		case exp_val of 
				NoPoints => exp_val
			...
			| _ => raise BadProgram("non-value on Shift")
			end
	...
	case e of 
	 	LineSegment (x1, y1, x2, y2) =>
			...
			then
				...
				(real_close(x1, x2) andalso y1 > y2) orelse (not (real_close(x1, x2)) andalso x1 > x2)
