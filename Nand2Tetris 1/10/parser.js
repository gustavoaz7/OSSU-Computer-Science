...
function parseDoStatement(tokens) {
  ...
  const subroutineCall = parseSubroutineCall(remainingTokens);
  children.push(...subroutineCall.children);
...
// pattern1                              | pattern2
// subroutineName '(' expressionList ')' | (className|varName) '.' subroutineName '(' expression ')'
...
function parseTerm(tokens) {
  ...
  if (isConstant(remainingTokens[0])) {            // (1)
      ...
      if (subroutineCall.children.length) {       // (4)
          children.push(...subroutineCall.children);
          remainingTokens = subroutineCall.remainingTokens;
      } else {                                     // (5)
          children.push(remainingTokens.shift());
